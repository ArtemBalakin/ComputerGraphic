...
Renderer::Renderer(HWND hwnd, int width, int height)
    : m_hwnd(hwnd), m_width(width), m_height(height), m_backgroundColor(Color::Black), m_useWireframe(false) {
...
// Установка квадратов с перемещением данных
void Renderer::SetSquares(std::vector<Square> squares) {
    if (m_squares == squares) return; // Избегаем избыточных обновлений
...
// Инициализация устройства и цепочки обмена
...
// Генерация геометрии (оптимизированные вычисления)
void Renderer::GenerateVerticesAndIndices() {
    ...
    for (const auto& square : m_squares) {
        ...
        const float left = (square.center.x - halfSize) * invWidth - 1.0f;
        const float right = (square.center.x + halfSize) * invWidth - 1.0f;
        const float top = 1.0f - (square.center.y - halfSize) * invHeight;
        const float bottom = 1.0f - (square.center.y + halfSize) * invHeight;
        ...
        for (int y = 0; y <= tess; ++y) {
            ...
            for (int x = 0; x <= tess; ++x) {
                ...
                m_vertices.emplace_back(left + (right - left) * u, top + (bottom - top) * v, 0.5f, 1.0f);
                m_vertices.push_back(color);
            }
        }

        // Генерация индексов (2 треугольника на подквадрат)
        const UINT base = static_cast<UINT>(m_vertices.size() / 2 - (tess + 1) * (tess + 1));
        for (int y = 0; y < tess; ++y) {
            for (int x = 0; x < tess; ++x) {
                const UINT i0 = base + y * (tess + 1) + x;
                m_indices.insert(m_indices.end(), {i0, i0 + 1, i0 + tess + 1, i0 + 1, i0 + tess + 2, i0 + tess + 1});
            }
        }
    }
}
